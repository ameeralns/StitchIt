...
async function generateHorizontalVideo(assFilePath: string, outputPath?: string): Promise<void> {
  ...
  const finalOutputPath = outputPath || `${inputFileName}_horizontal.mp4`;
  ...
  console.log(`Generating horizontal (16:9) video...`);
  console.log(`Input: ${assFilePath}`);
  console.log(`Output: ${finalOutputPath}`);

  // Create a more direct FFmpeg command
  const command = `ffmpeg -f lavfi -i "color=c=black:s=1920x1080:d=${duration}" -vf "ass=${assFilePath}:fontsdir=${fontsDir}" -c:v libx264 -preset medium -crf 23 -pix_fmt yuv420p -t ${duration} "${finalOutputPath}" -y`;
...
async function main() {
  ...
  if (args.length === 0) {
    console.log('Usage: npm run generate:horizontal <ass-file-path> [output-path]');
    console.log('Example: npm run generate:horizontal src/boi.ass my-video.mp4');
    process.exit(1);
  }
  ...
  try {
    await generateHorizontalVideo(assFilePath, outputPath);
  } catch (error) {
